  ...
  const render = () => {
    ...
    return (
      <Switch>
        ...
        <Route path="/routes" render={() => <RoutesContainer user={user} routerID={localStorage.getItem('currentRoute')} logOutHandler={logOutHandler} />} />
        <Route
          path="/profile/:id"
          render={(rProps) => {
            const id = parseInt(rProps.match.params.id);
