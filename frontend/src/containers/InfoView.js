...
import {
  SpotifyApiContext, Playlist, PlaylistTracks, Artist,
...
const InfoView = (props) => {
  ...
  return (
    <div>
      <ShowMap infoView getData={() => null} getCords={() => null} showMarkers={props.showMarkers} />
      <div id="plContainer">
          ...
          <PlaylistTracks id={props.playlist.split(':')[2]}>
            {
                    (tracks) => {
                      if (tracks.data) {
                        const mappedTracks = tracks.data.items.map((track) => (
